...
/*   Updated: 2024/01/18 13:04:18 by anqabbal         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*read_until_n_line(int fd, char *t_r)
{
	...
	buffer = my_calloc((size_t)BUFFER_SIZE + 1, sizeof(char));
	if (!buffer)
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0)
		return (NULL);
	if (t_r && my_strchr(t_r, '\n'))
		return (cut_the_rest(&t_r, &tmp, &line));
	else
	{
		line = read_until_n_line(fd, t_r);
		if (!line)
			return (t_r = NULL, NULL);
		else if (line && *line == '\0')
			return (line = to_join(line, NULL, 0), t_r = to_join(t_r, NULL, 0));
		t_r = cut_(line, 1);
		if (!t_r)
