...
/*   Updated: 2024/03/07 20:33:58 by anqabbal         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	left_move(t_win *v)
{
	...
	if ((v->line[p.y][p.i + 1] == 'E' || v->line[p.y][p.i] == 'E') && check_e_c(v->f_len, v->line, 'C'))
		exit(0);
...
void	up_move(t_win *v)
{
	...
	get_position(v->f_len, v->line, &p, 'P');
	if ((v->line[p.y - 1][p.i] == 'E' || v->line[p.y][p.i] == 'E') && check_e_c(v->f_len, v->line, 'C'))
		exit(0);
